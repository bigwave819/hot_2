...
export default async function ReservationsPage() {
  ...
  const [reservations, settings] = await Promise.all([listAllReservationsForStaff(), getHotelSettings()]);

  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Reservations" description="Review, confirm, and manage guest reservations." />
      <ReservationsManager reservations={reservations} currency={settings.currency} />
    </div>
  );
}